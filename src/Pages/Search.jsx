    ...
    <main className="flex-1 p-6">
     <div className="max-w-3xl mx-auto bg-white border border-gray-200 p-6">
      <div className="mb-4">
       <button
        onClick={() => router.push('/')}
        className="inline-flex items-center px-3 py-2 border border-gray-200 text-gray-700 hover:bg-gray-50 hover:text-[#C96442] transition-colors"
       >
      ...
      {error && (<div className="text-red-700 bg-red-50 border border-red-200 p-3 mb-4">{error}</div>)}
      <div className="space-y-8">
       <section>
        <h2 className="text-lg font-semibold text-gray-900 mb-3">Blogs ({items.length})</h2>
        <div className="space-y-4">
         {items.map((b) => (
          <article key={b._id} onClick={() => router.push(`/blog/${encodeURIComponent(b.slug || b._id)}`)} className="p-4 border border-gray-200 hover:bg-gray-50 cursor-pointer">
       ...
       <section>
        ...
        <div className="space-y-4">
         {qItems.map((qq) => {
          ...
          return (
           <article key={qq._id} onClick={() => router.push(`/question/${encodeURIComponent(qq.slug || qq._id)}`)} className="p-4 border border-gray-200 hover:bg-gray-50 cursor-pointer">
