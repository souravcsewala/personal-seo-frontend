  ...
  <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
   ...
   <div className="flex">
    ...
    <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${
     ...
     <div className="max-w-5xl mx-auto">
      {/* Profile Header */}
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-200 overflow-hidden mb-6 fade-in">
       {/* Cover Section */}
       <div className="h-32 bg-gradient-to-r from-[#C96442] via-[#B85538] to-[#A54F35]"></div>
       ...
       <div className="px-8 pb-8 -mt-16">
        <div className="flex flex-col md:flex-row items-start md:items-end space-y-4 md:space-y-0 md:space-x-6">
         ...
         {userData?.avatar ? (
          ...
          <div className="w-32 h-32 rounded-2xl bg-gradient-to-br from-[#C96442] to-[#A54F35] flex items-center justify-center text-4xl text-white font-bold border-4 border-white shadow-xl">
         ...
         <div className="flex-1 w-full">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
           <div>
            ...
            <p className="text-[#C96442] text-lg font-medium mt-8">{userData?.handle || ''}</p>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-4">
             ...
             {userData?.website && (
              <Link 
               ...
               className="flex items-center space-x-1 text-[#C96442] hover:text-[#B85538] transition-colors"
              >
               <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
           ...
           <button
            ...
            className="bg-gradient-to-r from-[#C96442] to-[#B85538] text-white px-6 py-2.5 hover:from-[#B85538] hover:to-[#A54F35] transition-all duration-200 shadow-md hover:shadow-lg hover:scale-[1.02] font-semibold flex items-center space-x-2"
           >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
           <div className="bg-white p-4 border border-gray-200 shadow-sm hover-lift">
          ...
          {/* Interests */}
          <div>
           <h3 className="text-lg font-bold text-gray-900 mb-3">Your Interests</h3>
           <div className="flex flex-wrap gap-2">
            {interests.length ? interests.map((c) => (
             <span 
              key={c._id} 
              className="px-3 py-1.5 bg-gradient-to-r from-[#C96442]/10 to-[#C96442]/5 text-[#C96442] text-sm font-semibold border border-[#C96442]/20"
             >
              {c.name || 'Category'}
             </span>
            )) : (
             <span className="text-gray-500 italic">No interests selected. Edit your profile to add interests.</span>
            )}
           </div>
          </div>
         </div>
       ...
       <div className="border-b border-gray-200 bg-gray-50/50">
        <nav className="flex space-x-1 px-2">
         <button
          ...
          className={`py-4 px-6 border-b-2 font-semibold text-sm transition-all duration-200 cursor-pointer relative ${
           activeTab === 'posts'
            ? 'border-[#C96442] text-[#C96442] bg-white'
         ...
         >
          <span className="flex items-center space-x-2">
           ...
           <span className={`px-2 py-0.5 text-xs font-bold ${
            activeTab === 'posts' ? 'bg-[#C96442]/20 text-[#C96442]' : 'bg-gray-200 text-gray-600'
           }`}>
            {userPosts.length}
           </span>
          </span>
         ...
         <button
          ...
          className={`py-4 px-6 border-b-2 font-semibold text-sm transition-all duration-200 cursor-pointer ${
           activeTab === 'questions'
            ? 'border-[#C96442] text-[#C96442] bg-white'
            : 'border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-100'
          }`}
         >
          <span className="flex items-center space-x-2">
           ...
           <span>Questions</span>
           <span className={`px-2 py-0.5 text-xs font-bold ${
            activeTab === 'questions' ? 'bg-[#C96442]/20 text-[#C96442]' : 'bg-gray-200 text-gray-600'
           }`}>
       ...
       <div className="p-6">
        {activeTab === 'posts' && (
         <div className="space-y-6">
          {userPosts.map((post) => (
           <div
            key={post.id}
            className="bg-white border border-gray-200 p-6 hover-lift transition-all duration-200 cursor-pointer group"
            onClick={() => router.push(`/blog/${encodeURIComponent(post.slug || post.id)}`)}
           >
            <div className="flex items-start justify-between mb-4">
             <h3 className="text-xl font-bold text-gray-900 flex-1 group-hover:text-[#C96442] transition-colors line-clamp-2">
              {post.title}
             </h3>
             <div className="flex items-center gap-2 ml-4">
              <button
               onClick={(e) => { e.stopPropagation(); router.push(`/edit-blog/${encodeURIComponent(post.id)}`); }}
               className="p-2 text-gray-400 hover:text-[#C96442] hover:bg-[#C96442]/10 transition-all duration-200 cursor-pointer"
               title="Edit post"
              >
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              ...
              <button
               ...
               className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200 cursor-pointer"
               title="Delete post"
              >
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {post.image && (
             <div className="relative w-full h-64 overflow-hidden mb-4 group-hover:opacity-90 transition-opacity">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100">
             <div className="flex flex-wrap gap-2">
              {post.tags.map((tag, index) => (
               <span
                key={index}
                className="bg-gradient-to-r from-[#C96442]/10 to-[#C96442]/5 text-[#C96442] px-3 py-1 text-sm font-semibold border border-[#C96442]/20"
               >
                {tag}
               </span>
              ))}
            ...
            <div className="flex items-center justify-between">
             <div className="flex flex-wrap gap-2">
              {poll.tags.map((tag, index) => (
               <span
                key={index}
                className="bg-[#C96442]/10 text-[#C96442] px-2 py-1 text-sm font-medium"
               >
                {tag}
               </span>
              ))}
        ...
        {activeTab === 'saved' && (
         <div className="text-center py-16">
          ...
          <p className="text-gray-600 mb-6">Posts you save will appear here.</p>
          <Link 
           href="/"
           className="inline-flex items-center space-x-2 bg-gradient-to-r from-[#C96442] to-[#B85538] text-white px-6 py-3 hover:from-[#B85538] hover:to-[#A54F35] transition-all duration-200 shadow-md hover:shadow-lg font-semibold"
          >
   ...
   {showProfileEditModal && (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
     <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-8 max-h-[90vh] overflow-y-auto scale-in">
      ...
      <div className="space-y-6">
       ...
       <div>
        ...
        <textarea
         ...
         className="w-full px-4 py-3 border-2 border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#C96442] focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white resize-none"
        />
       </div>
       ...
       <div>
        ...
        <div className="border-2 border-dashed border-gray-300 p-6 text-center bg-gray-50">
         ...
         >
          {editForm.profileimage ? 'Change Image' : 'Upload Image'}
         </label>
         {editForm.profileimage && (
          <p className="text-sm text-gray-600 mt-2">{editForm.profileimage.name}</p>
         )}
        </div>
       </div>
       {/* Interests in modal */}
       <div>
        <label className="block text-sm font-semibold text-gray-700 mb-3">Your Interests</label>
        <div className="flex flex-wrap gap-2">
         {allCats.map((c) => {
          ...
          return (
           <button
            ...
            className={`px-4 py-2 text-sm font-medium border-2 transition-all duration-200 ${
             selected 
              ? 'bg-gradient-to-r from-[#C96442] to-[#B85538] text-white border-[#C96442] shadow-sm' 
              : 'bg-white text-gray-700 border-gray-300 hover:border-[#C96442]/50 hover:bg-gray-50'
            }`}
           >
            {c.name}
           </button>
      ...
      <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-200">
       ...
       <button
        ...
        className="px-8 py-3 bg-gradient-to-r from-[#C96442] to-[#B85538] text-white hover:from-[#B85538] hover:to-[#A54F35] transition-all duration-200 shadow-lg hover:shadow-xl hover:scale-[1.02] font-semibold"
       >
        Save Changes
       </button>
      </div>
