   ...
   <div className="flex">
    ...
    <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${sidebarOpen ? 'lg:ml-64 ml-0' : 'ml-0'}`}>
     <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white shadow-sm border border-gray-200 p-4 mb-6">
       <div className="flex space-x-2">
        <button
         ...
         className={`px-4 py-2 text-sm font-medium cursor-pointer ${activeTab === 'followers' ? 'bg-[#C96442] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
         Followers
        </button>
        <button
         onClick={() => setActiveTab('following')}
         className={`px-4 py-2 text-sm font-medium cursor-pointer ${activeTab === 'following' ? 'bg-[#C96442] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
         Following
        </button>
       </div>
      ...
      {error && (
       <div className="border border-red-300 bg-red-50 p-4 text-red-700 mb-6">{error}</div>
      )}
         ...
         {list.map((u, idx) => (
          <li key={idx} className="p-4 flex items-center justify-between">
           <div className="flex items-center space-x-3">
            <span className="w-8 h-8 bg-gray-200 inline-block" />
