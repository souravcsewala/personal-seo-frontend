   ...
   <div className="flex">
    ...
    <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${
     ...
     <div className="max-w-3xl mx-auto">
      {loading && (
       <div className="border border-gray-200 p-4 text-gray-600">Loading poll...</div>
      )}
      {error && (
       <div className="border border-red-300 bg-red-50 p-4 text-red-700 mb-4">{error}</div>
      )}
      {poll && (
       <article className="bg-white shadow-sm border border-gray-200 p-6">
            ...
            {poll.userVoted ? (
             <>
              ...
              <div className="w-full bg-gray-200 h-2 mt-2">
               <div className={`h-2 ${isUserVote ? 'bg-[#C96442]' : 'bg-gray-400'}`} style={{ width: `${percent}%` }}></div>
              </div>
             </>
            ) : (
             <button
              ...
              className="w-full text-left p-3 border border-gray-200 hover:border-[#C96442] hover:bg-[#C96442]/5 transition-colors cursor-pointer disabled:opacity-60"
             >
              {option.text}
             </button>
            )}
