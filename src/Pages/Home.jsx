...
export default function Home({ initialFeed = [] }) {
 ...
 // Render poll options - Modern Design
     ...
     {poll.options.map((option, index) => {
      ...
      return (
       <div key={index} className="relative">
        <div className={`flex items-center justify-between p-4 border-2 transition-all duration-200 ${
         isUserVote ? 'border-[#C96442] bg-gradient-to-r from-[#C96442]/10 to-[#C96442]/5 shadow-sm' : 'border-gray-200 bg-gray-50'
        }`}>
         <div className="flex items-center space-x-3 flex-1">
          {isUserVote && (
           <svg className="w-5 h-5 text-[#C96442] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
           </svg>
          )}
          <span className={`font-semibold ${isUserVote ? 'text-[#C96442]' : 'text-gray-700'}`}>
           {option.text}
          </span>
         </div>
         <span className={`text-sm font-medium ml-3 ${isUserVote ? 'text-[#C96442]' : 'text-gray-600'}`}>
          {percentage}%
         </span>
        </div>
        <div className="w-full bg-gray-200 h-2.5 mt-3 overflow-hidden">
         <div 
          className={`h-full transition-all duration-500 ${
           isUserVote 
            ? 'bg-gradient-to-r from-[#C96442] to-[#B85538]' 
            : 'bg-gray-400'
          }`}
          style={{ width: `${percentage}%` }}
         ></div>
        </div>
        <div className="text-xs text-gray-500 mt-1 ml-1">{option.votes} {option.votes === 1 ? 'vote' : 'votes'}</div>
       </div>
      );
     })}
     <div className="pt-2 border-t border-gray-200">
      <p className="text-sm font-medium text-gray-700">Total: <span className="text-[#C96442]">{poll.totalVotes}</span> votes</p>
     </div>
    </div>
   );
  } else {
    ...
    <div className="space-y-3">
     {poll.options.map((option, index) => (
      <button
       ...
       className="w-full text-left p-4 border-2 border-gray-200 hover:border-[#C96442] hover:bg-gradient-to-r hover:from-[#C96442]/5 hover:to-[#C96442]/10 transition-all duration-200 cursor-pointer group"
      >
       <div className="flex items-center justify-between">
        <span className="font-medium text-gray-700 group-hover:text-[#C96442] transition-colors">
         {option.text}
        </span>
        <svg className="w-5 h-5 text-gray-400 group-hover:text-[#C96442] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
   ...
   <div className="flex">
    ...
    <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${
     ...
     <div className="max-w-4xl mx-auto">
      {announcement && (
       <div className="mb-6 bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200/50 px-6 py-4 shadow-sm hover-lift transition-all duration-200">
        <div className="flex items-center justify-between">
         <div className="flex items-center space-x-3 flex-1 min-w-0">
          <div className="flex-shrink-0">
           <svg className="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3.14a3.996 3.996 0 01.622 4.564M5.436 13.683A4 4 0 0018 13m-8 0V9a4 4 0 118 0v4m-8 0h8" />
           </svg>
          </div>
          <div className="flex-1 min-w-0">
           <a
            href={announcement?.linkUrl || '/announcements'}
            className="text-amber-900 font-semibold hover:text-[#C96442] transition-colors block truncate"
            target={announcement?.linkUrl && announcement.linkUrl.startsWith('http') ? '_blank' : undefined}
         ...
         <button 
          ...
          className="ml-4 text-xs font-medium text-amber-700 hover:text-[#C96442] transition-colors whitespace-nowrap"
         >
          View All →
         </button>
        </div>
      ...
      <div className="mb-8">
       ...
       <p className="text-gray-600 text-lg">Discover the latest trends, tips, and strategies from the SEO community.</p>
      </div>

      <div className="space-y-6">
       {feedLoading && (
        <div className="border border-gray-200 p-4"><LoadingIndicator /></div>
       )}
       {feedError && (
        <div className="border border-red-300 bg-red-50 p-4 text-red-700">{feedError}</div>
       )}
        ...
        <div className="mb-2 flex flex-wrap gap-2">
         {selectedCategory && (
          <button
           ...
           className="inline-flex items-center px-3 py-1 bg-[#C96442]/10 text-[#C96442] text-sm font-medium hover:bg-[#C96442]/20 transition-colors cursor-pointer"
          >
       ...
       {filteredContent.map((item) => (
        <article 
         key={item.id} 
         className="bg-white border border-gray-200 p-6 hover-lift transition-all duration-200 fade-in"
        >
         {/* Author & Meta Info */}
         <div className="flex items-center space-x-3 mb-4">
          <div 
           className="w-10 h-10 bg-gradient-to-br from-[#C96442] to-[#A54F35] flex items-center justify-center text-white font-semibold cursor-pointer hover:scale-110 transition-transform"
           onClick={() => {
            if (item.contentType === 'blog') {
          ...
          <div className="flex-1 min-w-0">
           <div className="flex items-center flex-wrap gap-2">
            <span 
             className="font-semibold text-gray-900 cursor-pointer hover:text-[#C96442] transition-colors truncate"
             onClick={() => {
              if (item.contentType === 'blog') {
               router.push(`/blogger/${encodeURIComponent(item.author.name)}`);
              }
            ...
            <span className="text-gray-500 text-sm hidden sm:inline">{item.publishDate}</span>
            <span className={`px-2.5 py-1 text-xs font-semibold ${
             item.contentType === 'blog' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
             item.contentType === 'question' ? 'bg-green-50 text-green-700 border border-green-200' :
           ...
           <div className="text-gray-500 text-sm sm:hidden mt-1">{item.publishDate}</div>
          </div>
         </div>

         {/* Title */}
         <h2 
          className="text-2xl font-bold text-gray-900 mb-3 cursor-pointer hover:text-[#C96442] transition-colors line-clamp-2"
          onClick={() => {
         ...
         {item.contentType === 'blog' ? (
          ...
          <div className="text-gray-700 leading-relaxed prose prose-sm max-w-none mb-4 line-clamp-3" dangerouslySetInnerHTML={{ __html: item.description || '' }} />
         )}

         {/* Featured Image */}
         {item.image && (
          <div 
           className="relative w-full h-64 overflow-hidden mb-4 cursor-pointer group"
           onClick={() => {
            if (item.contentType === 'blog') {
             const slug = item.slug || item.id;
             router.push(`/blog/${encodeURIComponent(slug)}`);
         ...
         <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div className="flex items-center space-x-4 text-gray-600">
           {item.contentType !== 'poll' && (
            <button 
             ...
             className={`flex items-center space-x-1.5 px-3 py-1.5 transition-all duration-200 ${
              item.isLiked 
               ? 'text-[#C96442] bg-[#C96442]/10' 
               : 'text-gray-600 hover:text-[#C96442] hover:bg-gray-50'
             }`}
            >
             <svg 
              className="w-5 h-5" 
           ...
           {item.contentType === 'poll' && (
            <div className="flex items-center space-x-1.5 px-3 py-1.5 text-gray-600">
             <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
             </svg>
             <span className="font-medium">{item.totalVotes || 0} votes</span>
            </div>
           )}
           <button 
            onClick={() => {
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 hover:text-[#C96442] hover:bg-gray-50 transition-all duration-200"
           >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
            <span className="font-medium">{item.comments || item.answers || 0}</span>
           </button>
           <button 
           ...
           <button 
            ...
            className={`px-3 py-1.5 transition-all duration-200 ${
             savedKeys.includes(keyFor(item)) 
              ? 'text-[#C96442] bg-[#C96442]/10' 
              : 'text-gray-600 hover:text-[#C96442] hover:bg-gray-50'
            }`}
           >
          ...
          {item.contentType === 'question' && (
           <button 
            ...
            className="bg-gradient-to-r from-[#C96442] to-[#B85538] hover:from-[#B85538] hover:to-[#A54F35] text-white px-4 py-2 transition-all duration-200 text-sm font-semibold shadow-sm hover:shadow-md hover:scale-[1.02]"
           >
            Answer
           </button>
          )}
    ...
    <aside className="hidden xl:block w-80 p-6 space-y-6">
     {/* Community Stats */}
     <div className="bg-white shadow-md border border-gray-200 p-6 hover-lift">
      <h3 className="text-lg font-bold text-gray-900 mb-5 flex items-center">
       <svg className="w-5 h-5 text-[#C96442] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
       </svg>
       Community Stats
      </h3>
     ...
     <div className="bg-white shadow-md border border-gray-200 p-6">
      <h3 className="text-lg font-bold text-gray-900 mb-5">What are you looking for?</h3>
      <div className="space-y-3">
       {isLoggedIn && (
        <div>
         ...
         {Array.isArray(followingOptions) && followingOptions.length > 0 ? (
          <select
           ...
           className="w-full px-3 py-2 border border-gray-300 focus:ring-2 focus:ring-[#C96442] focus:border-transparent"
          >
     ...
     {/* Popular Categories */}
     <div className="bg-white shadow-md border border-gray-200 p-6">
      <h3 className="text-lg font-bold text-gray-900 mb-5">Popular Categories</h3>
      <div className="space-y-3">
       {(showAllCategories ? allCategories : categories).map((category, index) => (
        <button 
         ...
         className="w-full flex items-center justify-between py-2 text-left hover:bg-gray-50 px-2 cursor-pointer"
        >
        ...
        return (
         <div key={(item?.doc?._id || index)} className="items-center justify-between">
          <div className="flex items-center space-x-2">
           ...
           {isBlog && catName && (
            <button onClick={go} className="ml-2 px-2 py-0.5 text-xs font-medium bg-[#C96442]/10 text-[#C96442] cursor-pointer">
             {catName}
            </button>
           )}
          </div>
   ...
   <div className="lg:hidden fixed bottom-6 right-6 z-50 fab-container">
    ...
    <div className={`absolute bottom-16 right-0 space-y-3 transition-all duration-300 ease-in-out ${
     ...
     <button
      ...
      className="bg-[#C96442] text-white p-3 shadow-lg hover:bg-[#C96442]/90 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#C96442] cursor-pointer"
    ...
    <button
     ...
     className={`bg-[#C96442] text-white p-4 shadow-lg hover:bg-[#C96442]/90 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#C96442] cursor-pointer ${
