       ...
       <div className="bg-white border border-gray-200 p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div>
          ...
          <select value={policy.blogs.policy} onChange={(e) => updateBlogsField("policy", e.target.value)} className="w-full px-3 py-2 border border-gray-300 ">
         ...
         <div>
          ...
          <input type="number" value={policy.blogs.maxExternalLinks ?? ""} onChange={(e) => updateBlogsField("maxExternalLinks", e.target.value === "" ? null : Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 " placeholder="null for unlimited" />
         </div>
         <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Max dofollow links</label>
          <input type="number" value={policy.blogs.maxDofollowLinks ?? 0} onChange={(e) => updateBlogsField("maxDofollowLinks", Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 " />
         </div>
         <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">If limits exceeded</label>
          <select value={policy.blogs.exceedMode} onChange={(e) => updateBlogsField("exceedMode", e.target.value)} className="w-full px-3 py-2 border border-gray-300 ">
         ...
         <div className="md:col-span-2">
          ...
          <input value={policy.blogs.relWhenNofollow || ""} onChange={(e) => updateBlogsField("relWhenNofollow", e.target.value)} className="w-full px-3 py-2 border border-gray-300 " />
         </div>
         <div className="flex items-center gap-2">
        ...
        <div>
         ...
         <input value={(policy.blogs.whitelist || []).join(", ")} onChange={(e) => updateBlogsField("whitelist", e.target.value.split(",").map((s) => s.trim()).filter(Boolean))} className="w-full px-3 py-2 border border-gray-300 " placeholder="example.com, partner.org" />
        </div>
        <div>
         <label className="block text-sm font-medium text-gray-700 mb-1">Blacklist (comma-separated)</label>
         <input value={(policy.blogs.blacklist || []).join(", ")} onChange={(e) => updateBlogsField("blacklist", e.target.value.split(",").map((s) => s.trim()).filter(Boolean))} className="w-full px-3 py-2 border border-gray-300 " placeholder="spam.com" />
        </div>
        <div>
         <label className="block text-sm font-medium text-gray-700 mb-1">Internal domains (comma-separated)</label>
         <input value={(policy.internalDomains || []).join(", ")} onChange={(e) => setInternalDomains(e.target.value.split(",").map((s) => s.trim()).filter(Boolean))} className="w-full px-3 py-2 border border-gray-300 " placeholder="farzacademy.com, blog.farzacademy.com" />
        </div>

        <div className="flex items-center gap-3 pt-2">
         <button onClick={savePolicy} className="px-4 py-2 bg-[#C96442] text-white hover:bg-[#C96442]/90 cursor-pointer">Save</button>
         <button onClick={createPolicy} className="px-4 py-2 border border-gray-300 text-gray-700 hover:bg-gray-50 cursor-pointer">Create</button>
         <button onClick={deletePolicy} className="px-4 py-2 bg-red-600 text-white hover:bg-red-700 cursor-pointer">Delete</button>
        </div>
       </div>
      )}
     </div>
