...
export default function QuestionApprovalPage() {
 ...
 useEffect(() => {
  ...
  async function load() {
   try {
    ...
    const resp = await axios.get(`${prodServerUrl}/questions`, {
     params,
 ...
 const handleSetStatus = async (id, nextStatus) => {
  try {
   await axios.put(`${prodServerUrl}/questions/${id}`, { status: nextStatus }, {
 ...
 const handleDelete = async (id) => {
  try {
   const ok = typeof window !== 'undefined' ? window.confirm('Delete this question permanently?') : true;
   if (!ok) return;
 ...
 const filtered = useMemo(() => {
  return (questions || []).filter(p => {
   ...
   const authorName = (p.author?.fullname || '').toLowerCase();
   const matchesSearch = !q || (p.title || '').toLowerCase().includes(q) || authorName.includes(q);
    ...
    <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${
     ...
     <div className="max-w-6xl mx-auto">
        ...
        <article key={item._id} className="bg-white shadow-sm border border-gray-200 p-5 flex flex-col h-full">
         <div className="flex-1">
          ...
          <div className="flex items-center space-x-2 mb-2">
           ...
           <span className="px-2 py-0.5 text-xs font-medium bg-green-100 text-green-800">Question</span>
          </div>
         ...
         <div className="mt-auto">
          {(item.status || 'pending') === 'pending' && (
           <div className="space-y-2">
            <Link href={`/admin/question/${item._id}`} className="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 transition-colors">View Question</Link>
      ...
      <div className="mt-6 flex items-center justify-between">
       ...
       <div className="text-sm text-gray-600">Page {page} • {total} questions</div>
