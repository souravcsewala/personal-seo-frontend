     ...
     <div className="max-w-5xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 p-6 space-y-4">
       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
         ...
         <input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#C96442] focus:border-transparent" placeholder="e.g. SEO Basics" />
        </div>
        <div className="md:col-span-2">
         <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
         <input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#C96442] focus:border-transparent" placeholder="Optional description" />
        </div>
       </div>
       <div className="flex items-center gap-3">
        <button type="submit" className="px-4 py-2 bg-[#C96442] text-white hover:bg-[#C96442]/90 transition-colors cursor-pointer">{isEditing ? "Update Category" : "Create Category"}</button>
        {isEditing && (
      ...
      <div className="bg-white border border-gray-200 p-6">
       <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
         <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by name/description" className="px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#C96442] focus:border-transparent" />
         <button onClick={() => { setPage(1); fetchList(); }} className="px-3 py-2 bg-gray-800 text-white hover:bg-gray-700 cursor-pointer">Search</button>
        </div>
        ...
        <div className="divide-y">
         {items.map((cat) => (
          <div key={cat._id} className="flex items-center justify-between py-3">
           ...
           <div className="flex items-center gap-3">
            <button onClick={() => handleEdit(cat)} className="px-3 py-1 border border-gray-300 text-gray-700 hover:bg-gray-50 cursor-pointer">Edit</button>
            <button onClick={() => handleDelete(cat._id)} className="px-3 py-1 bg-red-600 text-white hover:bg-red-700 cursor-pointer">Delete</button>
           </div>
          </div>
         ))}
        </div>
