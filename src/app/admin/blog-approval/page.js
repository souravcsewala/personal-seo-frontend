        ...
        <main className={`flex-1 p-6 transition-all duration-300 ease-in-out ${
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogs.map(item => (
                <article key={item._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 flex flex-col h-full">
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {item?.author?.profileimage?.url ? (
                        <Image src={item.author.profileimage.url} alt={item?.author?.fullname || 'Author'} width={32} height={32} className="w-8 h-8 rounded-full" />
                      ) : (
                  ...
                  <div className="mt-auto">
                    {(item.status || 'pending') === 'pending' && (
                      <div className="space-y-2">
                        <Link href={`/admin/post/${item._id}`} className="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg transition-colors">View Post</Link>
                        <div className="flex gap-2">
                          <button onClick={()=>handleSetStatus(item._id,'approved')} className="flex-1 border-2 border-green-400 text-green-600 hover:bg-green-50 px-4 py-2 rounded-lg transition-colors cursor-pointer">Approve</button>
                          <button onClick={()=>handleSetStatus(item._id,'rejected')} className="flex-1 border-2 border-red-400 text-red-600 hover:bg-red-50 px-4 py-2 rounded-lg transition-colors cursor-pointer">Decline</button>
                        </div>
                      </div>
                    )}
                    ...
                    {(item.status || 'pending') === 'approved' && (
                      <div className="space-y-2">
                        <Link href={`/admin/post/${item._id}`} className="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg transition-colors">View Post</Link>
                        <button onClick={()=>handleSetStatus(item._id,'rejected')} className="w-full border-2 border-red-400 text-red-600 hover:bg-red-50 px-4 py-2 rounded-lg transition-colors cursor-pointer">Decline</button>
                      </div>
                    )}
                    ...
                    {(item.status || 'pending') === 'rejected' && (
                      <div className="space-y-2">
                        <Link href={`/admin/post/${item._id}`} className="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg transition-colors">View Post</Link>
                        <button onClick={()=>handleSetStatus(item._id,'approved')} className="w-full border-2 border-green-400 text-green-600 hover:bg-green-50 px-4 py-2 rounded-lg transition-colors cursor-pointer">Approve</button>
                      </div>
                    )}
                  </div>
                </article>
