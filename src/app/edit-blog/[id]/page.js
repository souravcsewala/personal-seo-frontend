    ...
    <main className="flex-1 p-6">
     <div className="max-w-4xl mx-auto bg-white border border-gray-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
       ...
       <input id="title" value={form.title} onChange={(e)=>setForm(v=>({...v,title:e.target.value}))} placeholder="Enter an engaging title" className="w-full border border-gray-300 p-2" />

       {/* Meta Description removed: backend can auto-generate if absent */}

       <label htmlFor="category" className="block text-sm font-medium text-gray-700">Category *</label>
       <select id="category" value={form.category} onChange={(e)=>setForm(v=>({...v,category:e.target.value}))} className="w-full border border-gray-300 p-2">
       ...
       <input id="tags" value={form.tags} onChange={(e)=>setForm(v=>({...v,tags:e.target.value}))} placeholder="Tags (comma separated)" className="w-full border border-gray-300 p-2" />
         ...
         {form.featuredImage ? (
          <div className="space-y-3">
           ...
           <div className="flex items-center justify-center gap-3">
            ...
            <input id="imageAlt" value={form.imageAlt} onChange={(e)=>setForm(v=>({...v,imageAlt:e.target.value}))} placeholder="Alt text for the image" className="w-full max-w-md border border-gray-300 p-2" />
          ...
          <div>
           ...
           <label htmlFor="featuredImage" className="bg-[#C96442] text-white px-4 py-2 cursor-pointer">Choose Image</label>
          </div>
         )}
        </div>
       </div>
